...
int main(int argc, const char **argv){
    ...
    blockArray = createBlockArray(atoi(argv[create_tableIndex + 1]));
    pairArray = createPairArray(blockArray.size);
...
void removeRows(BlockArray *blockArray, int *indexArray, const char **argv, int argc){
    ...
    for(int i = 0; i < indexArrayLength; i++){
        ...
        freeLineWithIndex(&blockArray->storage[blockIndex], rowIndex);
    }
}
...
void removeBlockWithIndexes(BlockArray *blockArray, int *indexArray, const char **argv, int argc){
    ...
    for(int i = 0; i < indexArrayLength; i++){
        ...
        freeBlockWithIndex(blockArray, blockIndex);
    }
}
...
void runTest(int numOfFiles, int numOfRows){
    ...
    BlockArray blockArray = createBlockArray(numOfFiles);

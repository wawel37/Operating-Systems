...
int addNewClient(struct client_info client){
    if(fill<MAX_CLIENTS){
        clients[fill++]=client;
        return 0;
    }
...
void connectClient(int desc, int client_id, int second_client){
...
void clientExitHandler(int connected_client){
    struct message msg1;
...
void pingDisconnect(int i){
    ...
    if(connected_client !=-1){
        ...
        if(clients[connected_client].is_working==CONNECT){ 
            clientExitHandler(connected_client);  
        }
    }
...
// main loop ==========================================================
    ...
    while(1){
        ...
        if(event.data.fd==undesc || event.data.fd==netdesc){
            ...
            new_client.ping=0;
            sem_wait(&semaf);
            if(addNewClient(new_client)==ERROR){
    ...
    // CONNECT
            if(received.type==CONNECT){
                ...
                sem_wait(&semaf);
                int client_id = getClientIndexWithDesc(event.data.fd);
                struct message msg;

                if(getClientIndexWithName(received.name) == ERROR){
                    strcpy(clients[client_id].name,received.name);

                    int second_client = getFreeClientIndex(clients[client_id].desc);
                    ...
                    if(second_client == ERROR){
                        ...
                        connectClient(event.data.fd,client_id,second_client);
